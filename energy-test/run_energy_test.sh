...
# Arguments parsing
usage() {
  echo "Usage: $0 --d1 DSET1 --d2 DSET2 --nperm N --n1sub N --seed N --delta X.YY|--deltas X,Y,Z [--base NAME]"
  exit 2
}
DSET1=""; DSET2=""; N_PERM=""; N1_SUB=""; SEED=""; DELTA=""; DELTAS=""; BASE="energy_scan"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --d1)    DSET1="$2"; shift 2;;
    --d2)    DSET2="$2"; shift 2;;
...
# Require either DELTA or DELTAS
...
process_delta() {
  ...
  OUTFILE="${BASE}.csv"
  printf "%s,%s,%.12g,%.12g,%d,%d,%d\n" \
    "$(basename "$DSET1")" \
    "$(basename "$DSET2")" \
    "$PV_DELTA" "$PVAL" "$N_PERM" "$N1_SUB" "$SEED" >> "$OUTFILE"
}
